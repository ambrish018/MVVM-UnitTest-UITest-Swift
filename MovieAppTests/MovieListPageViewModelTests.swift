...
class MovieListPopularViewModelTests: XCTestCase {
...
//now playing
class MovieListNowPlayingViewModelTests: XCTestCase {
    var viewModelSut : MoviePopularPageViewModel!
    var nowPlayingDataFetcher : MockNowPayingMovieFether!
    override func setUp() {
        // Put setup code here. This method is called before the invocation of each test method in the class.
        viewModelSut = MoviePopularPageViewModel(dataFetcher: MockPopularMovieFether())
        viewModelSut.viewDidLoad()
        nowPlayingDataFetcher = MockNowPayingMovieFether()
    }
    
    override func tearDown() {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
        viewModelSut = nil
        nowPlayingDataFetcher = nil
    }
    func testOutPut(){
        XCTAssertEqual(viewModelSut.numberOfRows, nowPlayingDataFetcher.tableViewDataModel.count)
    }
    func testDatamodelForIndexPath(){
        let indexPath1 = IndexPath(row: 0, section: 1)
        XCTAssertEqual(viewModelSut.tableCellDataModelForIndexPath(indexPath1), nowPlayingDataFetcher.tableViewDataModel[0])
        let indexPath2 = IndexPath(row: 1, section: 1)
        XCTAssertEqual(viewModelSut.tableCellDataModelForIndexPath(indexPath2), nowPlayingDataFetcher.tableViewDataModel[1])
    }
    func testExample() {
        // This is an example of a functional test case.
        // Use XCTAssert and related functions to verify your tests produce the correct results.
...
class MockNowPayingMovieFether: NowplayingMovieDataFetcherProtocol {
    var movies : [MovieModel]!
    var tableViewDataModel =  [MovieCellDataModel]()
...
""".utf8)
    ...
    func fetchNowPlayingMovies(page: Int16, completion: @escaping ([MovieModel]?, String?) -> ()) {
        completion(movies,nil)
    }
